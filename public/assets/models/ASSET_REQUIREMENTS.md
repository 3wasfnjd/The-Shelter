...
**Status: all nine runtime GLBs supplied. Room/character assets are adapted from The-Shelter2; six puzzle stations are original lightweight replacements authorized by the user. Higher-quality art can replace these files while retaining node contracts.**
...
| `bunker-technician.glb` | (0,0,3.8), 0 | A 1.7 m technician, +Z forward, feet at origin, with `Idle` and `Walk` animation clips. Converted from Kenney Animated Characters Retro, with its included license. Efficient skinned mesh; materials match room art. |

## Acceptance before playable release

Run `npm run check:assets`. Current files pass. Then inspect every exported model in the application, verify pivots, UV orientation, clue legibility, cutaway walls, collisions and camera framing. Passing header/node checks does **not** establish visual quality or VR correctness. Asset budgets must be measured with the actual room on mobile and Quest; they cannot be certified from empty slots.
