...
export class LightingManager {
  constructor(scene,bunker) {
    this.fill=new THREE.HemisphereLight(0xc4d9df,0x65634a,1.7);scene.add(this.fill);
    this.key=new THREE.DirectionalLight(0xffe2ae,2.1);this.key.position.set(3,9,5);
    ...
    this.practicals=[[-2,3.3,-2],[2,3.3,2]].map(position=>{
      const light=new THREE.PointLight(0xffdca0,0,11,2);light.position.fromArray(position);bunker.root.add(light);return light;
    });
  ...
  update(puzzles,dt,mode) {
    ...
    for(const light of this.practicals)light.intensity=this.powerBlend*35*scale*scale;
